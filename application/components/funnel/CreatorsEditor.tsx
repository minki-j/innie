...
interface CreatorsEditorProps {
  funnelId: string;
  initialCreators: FunnelCreator[];
}

export function CreatorsEditor({ funnelId, initialCreators }: CreatorsEditorProps) {
  const [creators, setCreators] = useState<FunnelCreator[]>(initialCreators);
  ...
  const handleAdd = async () => {
    ...
    try {
      const res = await fetch(`/api/funnels/${funnelId}/creators`, {
        method: 'POST',
  ...
  const handleDelete = async (id: string) => {
    try {
      const res = await fetch(`/api/funnels/${funnelId}/creators?id=${id}`, {
        method: 'DELETE',
      });
      if (res.ok) {
  ...
  const startEditing = (creator: FunnelCreator) => {
