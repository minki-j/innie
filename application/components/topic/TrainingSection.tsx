...
export function TrainingSection({ topicId }: TrainingSectionProps) {
  ...
  const fetchTrainingInfo = useCallback(async () => {
    try {
      const res = await fetch(`/api/topics/${topicId}/training`);
      if (res.ok) {
  ...
  // Poll when training is in progress
  useEffect(() => {
    const isActive =
      info?.latestRun?.status === 'PENDING' ||
      info?.latestRun?.status === 'TRAINING';

    if (isActive) {
      pollRef.current = setInterval(async () => {
        const data = await fetchTrainingInfo();
        if (
          data?.latestRun?.status !== 'PENDING' &&
          data?.latestRun?.status !== 'TRAINING'
        ) {
          if (pollRef.current) {
  ...
  const handleStartTraining = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setActionResult({
          type: 'success',
          message: `${selectedMethod} training started! This may take a while.`,
        });
      ...
      setActionResult({
        type: 'error',
        message: 'Failed to connect to the training server.',
      });
    } finally {
      setIsStarting(false);
    }
  ...
  const handleCancelTraining = async () => {
    ...
    if (!window.confirm('Are you sure you want to cancel the current training run?')) {
      return;
    }
    ...
    try {
      const res = await fetch(
        `/api/training/${info.latestRun.id}/cancel`,
        { method: 'POST' },
      );
      ...
      if (res.ok) {
        ...
        setActionResult({
          type: 'error',
          message: data.error || 'Failed to cancel training.',
        });
      }
    } catch (error) {
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-gray-500">
              feedback{info.feedbackCount !== 1 ? 's' : ''} available
            </span>
          </div>
          {info.feedbackCount < info.minFeedbackRequired && (
            <p className="text-xs text-amber-600 mt-1">
              At least {info.minFeedbackRequired} feedbacks with transcripts are
        ...
        <div className="border-t border-gray-100 pt-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-gray-400">
              {formatDate(info.latestRun.createdAt)}
            </span>
          </div>

          {info.latestRun.datasetSize && (
            <p className="text-xs text-gray-500 mt-1">
              Trained on {info.latestRun.datasetSize} datapoints
            </p>
          )}

          {info.latestRun.status === 'COMPLETED' && info.latestRun.completedAt && (
            <p className="text-xs text-green-600 mt-1">
              Completed on {formatDate(info.latestRun.completedAt)}
            </p>
          )}

          {info.latestRun.status === 'FAILED' && info.latestRun.error && (
            <p className="text-xs text-red-600 mt-1">
              Error: {info.latestRun.error}
            </p>
          )}

          {info.latestRun.status === 'CANCELLED' && (
            <p className="text-xs text-gray-500 mt-1">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-blue-800">
                {info.latestRun?.method} training is in progress...
              </p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full mx-4 p-6">
            <h3 className="text-lg font-semibold text-gray-900">
              Start {selectedMethod} Training?
            </h3>
            <div className="mt-3 space-y-2">
              <p className="text-sm text-gray-600">
                This will start <span className="font-medium">{METHOD_INFO[selectedMethod].label}</span> training
                using your{' '}
              ...
              <div className="bg-gray-50 rounded-lg px-3 py-2 text-sm">
                <div className="flex justify-between text-gray-600">
                  ...
                  <span className="font-medium text-gray-900">
                    {formatTokens(selectedTokens)} tokens
                  </span>
                </div>
                {selectedMethod === 'RLVR' && (
                  <p className="text-xs text-gray-400 mt-1">
                    50 steps &times; 2 groups/batch &times; 8 rollouts/group
                  </p>
                )}
                {selectedMethod === 'SFT' && (
                  <p className="text-xs text-gray-400 mt-1">
                    1 epoch &times; {info.feedbackCount} datapoints
                  </p>
                )}
              </div>
              <p className="text-xs text-gray-500">
                Training may take several minutes. You can cancel it at any time.
              </p>
            </div>
